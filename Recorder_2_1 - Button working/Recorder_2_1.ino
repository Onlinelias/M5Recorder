...
static const uint8_t transportButtonPin = 8;  // Unit Key button signal on CoreS3 Port B.
static const uint8_t transportButtonLedPin = 9;  // Unit Key SK6812 LED signal on CoreS3 Port B.
...
static Adafruit_NeoPixel keyLed(1, transportButtonLedPin, NEO_GRB + NEO_KHZ800);
static bool keyLedReady = false;

static const uint8_t monitorVolume = 30;
static bool monitorEnabled = false;
static bool useInputLine2 = false;
static uint8_t micAdcVolume = 45;
...
static void setStatusLed(AppState state)
{
    if (!keyLedReady)
    {
        return;
    ...
    {
    case APP_READY:
        color = keyLed.Color(0, 18, 0);
        break;
    case APP_STARTING:
    case APP_STOPPING:
        color = keyLed.Color(24, 12, 0);
        break;
    case APP_RECORDING:
        color = keyLed.Color(28, 0, 0);
        break;
    case APP_SAVED:
        color = keyLed.Color(0, 36, 0);
        break;
    case APP_ERROR:
        color = keyLed.Color(48, 0, 0);
        break;
    }

    keyLed.setPixelColor(0, color);
    keyLed.show();
}
...
static void configureTransportButtonPins()
{
    pinMode(transportButtonPin, INPUT_PULLUP);
    if (transportButtonLedPin != transportButtonPin)
    {
        keyLed.setPin(transportButtonLedPin);
        keyLed.begin();
        keyLed.setBrightness(48);
        keyLedReady = true;
        setStatusLed(appState);
    }
...
static void printButtonStatus()
{
    int raw = digitalRead(transportButtonPin);
    Serial.printf("BUTTON gpio=%u raw=%s pressed=%d led_gpio=%u\n",
                  transportButtonPin,
                  raw == LOW ? "LOW" : "HIGH",
                  raw == LOW,
                  transportButtonLedPin);
}
