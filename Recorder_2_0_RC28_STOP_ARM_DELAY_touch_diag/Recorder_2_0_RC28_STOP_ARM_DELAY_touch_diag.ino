...
static TouchDiagState startTouchDiag;
static TouchDiagState stopTouchDiag;

static bool isEdgeTouch(const lgfx::touch_point_t &t)
{
    return (t.x < 200) ||
           (t.x > 3000) ||
           (t.y < 200) ||
...
static void logTouchContinuity(
    const char *label,
    TouchDiagState &state,
    const lgfx::touch_point_t &t,
    int rawCount)
{
    uint32_t now = millis();
    const char *kind = "FIRST";
    uint32_t releaseGap = 0;
    int dx = 0;
    int dy = 0;
    ...
    else if (state.lastValidMs != 0)
    {
        kind = "REASSERT_AFTER_RELEASE";
        releaseGap = state.lastReleaseMs == 0 ? 0 : now - state.lastReleaseMs;
        dx = t.x - state.lastX;
        dy = t.y - state.lastY;
    }
    ...
    Serial.printf(
        "%s VALID seq=%lu kind=%s id=%d x=%d y=%d size=%d raw=%d prevId=%d dx=%d dy=%d releaseGapMs=%lu\n",
        label,
        (unsigned long)state.validSeq,
        kind,
        t.id,
        t.x,
        t.y,
        ...
        dx,
        dy,
        (unsigned long)releaseGap);
}

static bool scanValidTouch(
    const char *label,
    TouchDiagState &state,
    ...
    int rawCount)
{
    ...
    Serial.printf("%s RAW count=%d\n", label, rawCount);

    for (int i = 0; i < rawCount; ++i)
    {
        auto &t = points[i];
        bool edgeTouch = isEdgeTouch(t);

        Serial.printf(
            "%s RAW[%d] id=%d x=%d y=%d size=%d edge=%d\n",
            label,
            i,
            t.id,
            t.x,
    ...
    if (validTouch)
    {
        ...
        logTouchContinuity(label, state, t, rawCount);

        state.wasValid = true;
        state.lastId = t.id;
        state.lastX = t.x;
        state.lastY = t.y;
        state.lastValidMs = millis();
    }
    else if (state.wasValid)
    {
        state.wasValid = false;
        state.lastReleaseMs = millis();
        Serial.printf(
            "%s RELEASE seq=%lu lastId=%d lastX=%d lastY=%d raw=%d\n",
            label,
            (unsigned long)state.validSeq,
            state.lastId,
            state.lastX,
...
bool validTouch = scanValidTouch(
    "START",
    startTouchDiag,
    rawtp,
    rawCountStart);
...
// ---------------------------------
    ...
    bool stopValidTouch = scanValidTouch(
        "STOP",
        stopTouchDiag,
        stoptp,
        stopCount);
